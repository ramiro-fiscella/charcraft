...
const CharacterDetails = () => {
  ...
  const [owner, setOwner] = useState(null); // Datos del propietario del personaje
  const navigate = useNavigate();

  // Placeholder para simular el estado de autenticación.
  // En tu nuevo sistema, obtendrás el usuario desde tu contexto o hook de autenticación.
  const isLoggedIn = true;
  const user = isLoggedIn ? { uid: 'dummy_uid' } : null; // Usamos uid en lugar de sub
  ...
  useEffect(() => {
    const fetchCharacter = async () => {
      try {
        ...
        // Obtiene el usuario (propietario) que creó el personaje
  ...
  // Se ajusta la función para actualizar el estado de habilidades.
  ...
  // Ahora comparamos owner.uid con user.uid
  const isOwner = user && owner.uid === user.uid;

  console.log('Usuario actual:', user, 'ID del creador:', character.user_id);
        ...
        <div>
          <div id="skills" className="w-80 p-2">
            <ul className="mx-2 mt-0 p-3 shadow-lg shadow-neutral-950 rounded-xl border-t-2 border-neutral-800">
              ...
              {/* Se repite el mismo patrón para las demás habilidades */}
            </ul>
          </div>
        </div>
      </div>
